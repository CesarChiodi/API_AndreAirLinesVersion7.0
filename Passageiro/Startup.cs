...
namespace MicroServicoPassageiroAPI
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection servico)
        {
            ...
            var key = Encoding.ASCII.GetBytes(ClasseConfiguracaoPassageiro.Secret);
            servico.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "Passageiro", Version = "v1" });
            });
            ...
            servico.Configure<PassageiroAPI>(
                Configuracao.GetSection(nameof(PassageiroAPI)));

            servico.AddSingleton<IPassageiroAPI>(sp =>
                sp.GetRequiredService<IOptions<PassageiroAPI>>().Value);

            servico.AddSingleton<ServicoPassageiro>();
        }
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Passageiro v1"));
            }
